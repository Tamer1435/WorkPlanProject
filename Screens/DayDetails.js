...
const DayDetails = () => {
  ...
  const addGroupToDay = () => {
    setGroups([...groups, { classes: [], students: [], title: `קבוצה ${groups.length + 1}`, place: '', time: '', teacher: '' }]);
  ...
  const toggleStudentSelection = (studentName) => {
    ...
    if (isSelected) {
      group.students = group.students.filter(student => student !== studentName);
    } else {
  ...
  const handleTimeChange = (event, selectedDate, index, isStart) => {
    if (selectedDate) {
      ...
      updatedGroups[index].time = `${updatedGroups[index].startTime.getHours()}:${updatedGroups[index].startTime.getMinutes().toString().padStart(2, '0')} - ${updatedGroups[index].endTime.getHours()}:${updatedGroups[index].endTime.getMinutes().toString().padStart(2, '0')}`;
      setGroups(updatedGroups);
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView>
        {groups.map((group, index) => (
          <View key={index} style={styles.groupContainer}>
            <TouchableOpacity style={styles.deleteButton} onPress={() => deleteGroup(index)}>
            ...
            <View style={styles.row}>
              <Text style={styles.timeText}>{group.time.split(' - ')[0] || 'בחר זמן'}</Text>
              <TouchableOpacity onPress={() => setShowStartTimePicker(index)}>
                <Text style={[styles.timeText, styles.placeholderText]}>{group.time.split(' - ')[0] ? '' : 'בחר זמן'}</Text>
              </TouchableOpacity>
              ...
              {showStartTimePicker === index && (
                <DateTimePicker
                  ...
                  is24Hour={true}
                  display="default"
                  onChange={(event, date) => handleTimeChange(event, date, index, true)}
                />
              )}
            </View>
            <View style={styles.row}>
              <Text style={styles.timeText}>{group.time.split(' - ')[1] || 'בחר זמן'}</Text>
              <TouchableOpacity onPress={() => setShowEndTimePicker(index)}>
                <Text style={[styles.timeText, styles.placeholderText]}>{group.time.split(' - ')[1] ? '' : 'בחר זמן'}</Text>
              </TouchableOpacity>
              ...
              {showEndTimePicker === index && (
                <DateTimePicker
                  ...
                  is24Hour={true}
                  display="default"
                  onChange={(event, date) => handleTimeChange(event, date, index, false)}
                />
              )}
            </View>
            <View style={styles.row}>
              <TouchableOpacity onPress={() => { setSelectedGroupIndex(index); setTeacherModalVisible(true); }}>
                <Text style={styles.teacherText}>{group.teacher || 'בחר מורה'}</Text>
              </TouchableOpacity>
              <Text style={styles.label}>:מורה</Text>
            </View>
            <TouchableOpacity style={styles.classButton} onPress={() => { setSelectedGroupIndex(index); setClassSelectionModalVisible(true); }}>
            ...
            <View style={styles.classContainer}>
              {group.classes.map((className) => (
                <View key={className} style={styles.classSection}>
                  <TouchableOpacity style={styles.classButton} onPress={() => selectClassForGroup(className)}>
                    <Text style={styles.classButtonText}>{className} ({group.students.filter(student => allStudents[className].includes(student)).length})</Text>
                  </TouchableOpacity>
                  <View style={styles.studentList}>
                    {group.students.filter(student => allStudents[className].includes(student)).map(student => (
                      <Text key={student} style={styles.studentName}>{student}</Text>
                    ))}
                  </View>
                </View>
              ))}
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            {['6th', '7th', '8th', '9th', '10th', '11th', '12th'].map((className) => (
              <TouchableOpacity
                key={className}
                style={[styles.modalOption, selectedClasses.includes(className) && styles.selectedClass]}
                onPress={() => toggleClassSelection(className)}
              >
                <Text style={styles.modalOptionText}>{className}</Text>
              </TouchableOpacity>
            ))}
            <TouchableOpacity onPress={finalizeClassSelection} style={styles.modalCloseButton}>
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>בחר תלמידים לכיתה {selectedClass}</Text>
            <FlatList
              ...
              renderItem={({ item: student }) => (
                ...
                >
                  <Text style={[styles.modalOptionText, {
                    color: (groups[selectedGroupIndex] && groups[selectedGroupIndex].students.includes(student)) ? 'green' : 'black'
                  }]}>{student}</Text>
                </TouchableOpacity>
              )}
            />
